...
- `linear-requirement-planner` 位于 `skills/linear-requirement-planner/SKILL.md`，用于把 Linear brief 需求结合项目实际情况转成中文实现计划；存在阻塞型缺口时必须先 `AskQuestion` 澄清，计划完成后会把摘要和预期 GitHub 链接评论回 Linear，用 `save_document` 把全文同步到关联 Linear Document，并在回复中附带本地计划文档路径（不上传重复附件）。
